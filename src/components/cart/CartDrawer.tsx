    ...
    >
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto overscroll-contain p-5 space-y-4">
          ...
          {cart.length > 0 && <div className="bg-white border border-[#E6D5B8] rounded-xl p-3.5 text-xs"><button onClick={() => navigate('contact')} className="flex items-center gap-2 underline"><Gift className="h-4 w-4" />Enquire about gift packaging</button></div>}

        </div>

        {cart.length > 0 && cartIssue && <p role="alert" className="px-4 py-3 text-xs text-red-700">{cartIssue}</p>}
        ...
        {cart.length > 0 && (
          <div className="p-5 border-t border-[#E6D5B8] bg-white space-y-3 shadow-lg">
            ...
            <div className="flex items-center justify-center gap-2 text-[10px] text-stone-400">
              <ShieldCheck className="w-3.5 h-3.5 text-[#C5A059]" /> Review your selections before checkout
            </div>
          </div>
        )}
      </motion.div>
