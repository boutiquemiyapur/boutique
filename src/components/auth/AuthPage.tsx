...
export const AuthPage: React.FC<{ mode: Mode }> = ({ mode: initialMode }) => {
  ...
  return <div className="min-h-screen bg-[#fbf9f7] p-0 lg:grid lg:place-items-center lg:p-8"><section className="grid min-h-screen w-full max-w-[1280px] overflow-hidden border border-[#d0c4bc] bg-white lg:min-h-[80vh] lg:grid-cols-2">
    <div className="relative hidden min-h-full overflow-hidden lg:block"><img src={imageUrl} alt="AB Collection" className="absolute inset-0 h-full w-full object-cover grayscale-[25%]" /><div className="absolute inset-0 bg-[#30302f]/20" /><button onClick={() => navigate('home')} className="absolute left-8 top-8 max-w-[min(100%-4rem,22rem)]"><BrandMark inverse /></button></div>
    <div className="flex min-h-screen flex-col justify-center px-7 py-12 sm:px-14 lg:min-h-0 lg:px-20"><button onClick={() => navigate('home')} className="mb-12 max-w-full self-start lg:hidden"><BrandMark /></button><div className="mx-auto w-full max-w-md"><div className="mb-10"><h1 className="font-serif text-4xl text-[#1b1c1b]">{title}</h1><p className="mt-3 text-base text-[#4d453f]">{subtitle}</p></div>{error && <div role="alert" className="mb-5 border border-[#ba1a1a]/30 bg-[#ffdad6]/50 p-3 text-sm text-[#93000a]">{error}</div>}{notice && <div role="status" className="mb-5 border border-[#685c53]/30 bg-[#f5f3f1] p-3 text-sm text-[#4f453c]">{notice}</div>}
